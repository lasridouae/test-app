...
export class EditProductComponent implements OnInit {
  ...
  /* Subscription */
  public EditCategorySup: Subscription = new Subscription();
  public ParentCategorySup: Subscription = new Subscription();
  public UploadeImageSup: Subscription = new Subscription();
  public CurrentProductSup: Subscription = new Subscription();
  ...
  */
  ngOnDestroy(): void {
    this.EditCategorySup.unsubscribe();
    this.ParentCategorySup.unsubscribe();
    this.CurrentProductSup.unsubscribe();
  ...
  editProduct(form) {
    if (form.valid) {
      ...
      this.EditCategorySup = this.dataService.put('products', this.product).subscribe(async (res: any) => {
        if (res.success) {
          this.dataService.toastrSuccess("Product has been updated");
          this.content_loaded = false;
          setTimeout(async () => {
            // Redirect to list
        ...
        async (error: any) => {
          console.log(error)
          this.dataService.toastrDanger("Error while deleting Product, " + error);
        });
    }
  }
  ...
  async getProduct() {
    this.CurrentProductSup = this.dataService.get('products/' + this.paramId).subscribe(async (res: any) => {
      if (res.success) {
    ...
    this.ParentCategorySup = this.dataService
      ...
      .subscribe(async (res: any) => {
        if (res.success) {
          ...
          this.selectedOptionParent = this.getSelectedCategoryParent(this.categories, this.product.category_id ? this.product.category_id : 0  );
