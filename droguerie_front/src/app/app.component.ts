...
export class AppComponent implements OnInit, OnDestroy {
  ...
  ngOnInit(): void {
    ...
    if(localStorage.getItem('currentUser')){
      this.dataService.get('settings').subscribe(async (res: any) => {
        let data = null;
        if (res.success) {
          data = await res.data;
          this.setting = data;
          this._coreConfigService.setConfig({ app: { 
            appName: this.setting.appName, 
            appTitle: this.setting.appTitle, 
            appLogoImage: this.setting.appLogoImage ? this.SETTING_IMAGE_PATH + this.setting.appLogoImage : 'assets/images/logo/logo.svg' ,  
            appBgLoginImage: this.setting.appBgLoginImage ? this.SETTING_IMAGE_PATH + this.setting.appBgLoginImage : 'assets/images/pages/login-v2.svg' ,  
          } }, { emitEvent: true });
